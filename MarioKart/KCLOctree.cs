...
namespace MarioKart
{
	public class KCLOctree
    {
        ...
        public void Write(EndianBinaryWriter er)
        {
            ...
            while (nodes.Count > 0)
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i < 8; i++)
                        nodes.Enqueue((node.SubNodes[i], er.BaseStream.Position, i * 4));
                    er.Write(new uint[8], 0, 8);
                }
            }

            //traverse leaves (depth first order)
            var indexData   = new LinkedList<IndexData>();
            var nodeIdxData = new Dictionary<KCLOctreeNode, IndexData>();
            ...
            while (nodes2.Count > 0)
            {
                ...
                if (node.IsLeaf)
                {
                    ...
                    while(dataNode != null)
                    {
                        ...
                        if (data.Indices.SequenceEqual(node.Triangles))
                        {
                    ...
                    if (!done)
                    {
                        var data = new IndexData(node.Triangles);
                        indexData.AddLast(data);
                        nodeIdxData.Add(node, data);
                    }
                }
            ...
            foreach (var data in indexData)
            {
                ...
                if (data.Indices.Length > 0 || idx == 0)
                {
                    foreach (var v in data.Indices)
                        er.Write((ushort) (v + 1));
                    er.Write((ushort) 0);
                }
                else
            ...
            //write the leaf pointers
            foreach (var pair in nodeIdxData)
            {
        ...
        public class KCLOctreeNode
        {
            ...
            private static bool AxisTest(float a1, float a2, float b1, float b2, float c1, float c2, float half)
            {
                float p = a1 * b1 + a2 * b2;
                float q = a1 * c1 + a2 * c2;
            ...
            private static bool TriCubeOverlap(Triangle t, Vector3 position, float boxSize)
            {
                ...
                if (Math.Min(Math.Min(v0.X, v1.X), v2.X) > half || Math.Max(Math.Max(v0.X, v1.X), v2.X) < -half)
                    return false;
